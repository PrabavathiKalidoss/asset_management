...
class ApiBaseService extends ApiBaseHelper {
  ...
  List<String> strapiStaticPath = [
   // "/strapi/pages/notice-board",
     // "/strapi/pages/home-work",
    //"/strapi/pages/study-material",
   // "/strapi/pages/settings",
   // "/strapi/pages/photos",
    //"/strapi/pages/gallery",
  ];
  ...
  Future<HTTP.Response?> _sendAsync(String method, String endPoint, Object? body, { Map<String, String>? queryParams, bool authenticated = true}) async {
    ...
    if (strapiStaticPath.where((element) => endPoint.startsWith(element)).toList().isNotEmpty) {
      basePath = "http://vee2care.strapi.vgts.tech/api";
    }
    String path =  basePath + endPoint ;

    var url = Uri.parse( path + (queryParams != null ? Uri(queryParameters: queryParams).query : ""));

    Logger.d("PARAMS ${body}");
