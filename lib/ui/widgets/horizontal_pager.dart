...
class _HorizontalCardPagerState extends State<HorizontalCardPager> {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      double viewHeight = viewWidth / 3.0;
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < widget.items!.length; i++) {
      ...
      Widget card = Positioned.directional(
          ...
          start: _getStartPosition(cardWidth, widget.cardMaxWidth, widget.viewWidth!, i, selectedIndex!),
          child: Opacity(
            opacity: _getOpacity(i),
            child: Container(
              child: Center(
                child: widget.items![i].buildWidget((i.toDouble() - selectedIndex!).abs()),
              ),
              width: cardWidth,
              height: cardHeight,
            ),
